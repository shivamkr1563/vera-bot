...
load_dotenv()
LLM_API_KEY = os.getenv("OPENROUTER_API_KEY") or LLM_API_KEY
...
class DatasetLoader:
    ...
    def load(self) -> bool:
        try:
            ...
            # Load merchants from nested "merchants" key
            merchants_path = self.dataset_dir / "merchants_seed.json"
            if merchants_path.exists():
                data = json.load(open(merchants_path))
                items = data.get("merchants", [])
                for item in items:
                    if "merchant_id" in item:
                        self.merchants[item["merchant_id"]] = item

            # Load triggers from nested "triggers" key
            triggers_path = self.dataset_dir / "triggers_seed.json"
            if triggers_path.exists():
                data = json.load(open(triggers_path))
                items = data.get("triggers", [])
                for item in items:
                    if "id" in item:
                        self.triggers[item["id"]] = item

            # Load customers from nested "customers" key
            customers_path = self.dataset_dir / "customers_seed.json"
            if customers_path.exists():
                data = json.load(open(customers_path))
